...
public Best chooseMove(int side,int alpha,int beta) {
	 ...
	 if( side == COMPUTER && reply.val > alpha
	 || side == HUMAN && reply.val < beta ) {
	 if(side==COMPUTER) alpha=reply.val;
	 if(side==HUMAN) beta=reply.val;
	 ...
	 if(side==COMPUTER) 
		 return new Best( alpha, bestRow, bestColumn );
	 else return new Best(beta,bestRow,bestColumn);
